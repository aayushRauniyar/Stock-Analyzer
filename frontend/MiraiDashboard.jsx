...
export default function App() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      // Call backend to get pre-transformed data
      const res = await fetch(`/api/market-data?t=${Date.now()}`);
      if (!res.ok) {
        console.warn("Backend not running — falling back to seed data.");
        setEtf(prev => ({ ...prev, ...SEED }));
        throw new Error("Backend unavailable");
      }
      const snapshot = await res.json();
      if (snapshot && snapshot.data) {
        setEtf(prev => ({ ...prev, ...snapshot.data }));
        if (snapshot.market_open !== undefined) setMktOpen(snapshot.market_open);
        if (snapshot.fetched_at_aest) setLastUpdate(snapshot.fetched_at_aest);
      }

      // Also fetch cached AI signals
      try {
        const sigRes = await fetch("/api/signals");
        if (sigRes.ok) {
          const sigData = await sigRes.json();
          if (sigData.signals) setSig(prev => ({ ...prev, ...sigData.signals }));
        }
      } catch (_) {}
    } catch (e) {
  ...
  // ── trigger full refresh (data + AI analysis) via backend ──────────────────
  const refreshAll = useCallback(async () => {
    setLoading(l => ({ ...l, SPY: true, QQQ: true, VTI: true,
      s_SPY: true, s_QQQ: true, s_VTI: true }));
    try {
      const res = await fetch("/api/refresh", { method: "POST" });
      if (res.ok) {
        const result = await res.json();
        if (result.market_data) setEtf(prev => ({ ...prev, ...result.market_data }));
        if (result.signals) setSig(prev => ({ ...prev, ...result.signals }));
        if (result.market_open !== undefined) setMktOpen(result.market_open);
    ...
    setLoading(l => ({ ...l, SPY: false, QQQ: false, VTI: false,
      s_SPY: false, s_QQQ: false, s_VTI: false }));
  }, [fetchAll]);
  ...
  useEffect(() => {
    ...
    es.addEventListener("update", (e) => {
      try {
        ...
        if (data.market_data) setEtf(prev => ({ ...prev, ...data.market_data }));
        if (data.signals) setSig(prev => ({ ...prev, ...data.signals }));
    ...
    es.addEventListener("refresh", (e) => {
      try {
        ...
        if (data.market_data) setEtf(prev => ({ ...prev, ...data.market_data }));
        if (data.signals) setSig(prev => ({ ...prev, ...data.signals }));
        if (data.market_open !== undefined) setMktOpen(data.market_open);
        if (data.refreshed_at) setLastUpdate(data.refreshed_at);
      } catch (_) {}
    });
      ...
      <div style={{ flex: 1, display: "flex",
        ...
        <header style={{
          ...
          <div style={{ marginLeft: "auto", display: "flex",
            ...
            {/* Refresh all */}
            <button onClick={refreshAll} className="hvr" disabled={!!Object.values(loading).find(Boolean)}
              style={{
                padding: "6px 16px",
                fontFamily: "'Shippori Mincho B1',serif",
                fontSize: 9, letterSpacing: "0.14em", textTransform: "uppercase",
